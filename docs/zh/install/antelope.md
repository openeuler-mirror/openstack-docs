...
首先准备三个|openEuler 24.03 LTS SP4环境，根据您的环境，下载对应的镜像并安装即可：[ISO镜像](https://repo.openeuler.org/openEuler-24.03-LTS-SP2/ISO/)、[qcow2镜像](https://repo.openeuler.org/openEuler-24.03-LTS-SP2/virtual_machine_img/)。
...
1. 配置 |openEuler 24.03 LTS SP4 官方 yum 源，需要启用 EPOL 软件仓以支持 OpenStack
...
2. 配置对接华为云provider的信息
    ...
    - 一个安全组，名字默认是`oos`
    - 一个openEuler镜像，名称格式是openEuler-%(release)s-%(arch)s，例如`openEuler-24.03-sp4-arm64`
    - 一个VPC，名称是`oos_vpc`
...
4. 华为云上面创建一台|openEuler 24.03 LTS SP4的x86_64虚拟机，用于部署`all in one` 的 OpenStack
